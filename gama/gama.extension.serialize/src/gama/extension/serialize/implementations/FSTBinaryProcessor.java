/*******************************************************************************************************
 *
 * FSTBinaryProcessor.java, in gama.extension.serialize, is part of the source code of the GAMA modeling and simulation
 * platform.
 *
	...
	protected void registerSerialisers(final FSTConfiguration conf) {
		...
		register(conf, SerialisedAgent.class, new FSTIndividualSerialiser<SerialisedAgent>() {
			...
			public void serialise(final FSTObjectOutput out, final SerialisedAgent o) throws Exception {
				DEBUG.OUT("serialise biary SerialisedAgent " + o);
				DEBUG.OUT("serialise biary SerialisedAgent " + o.uuid());
				DEBUG.OUT("serialise biary SerialisedAgent string" + o.uuid().toString());
				DEBUG.OUT("serialise biary clclclclclcll " + o.uuid().getClass());
			...
			public SerialisedAgent deserialise(final IScope scope, final FSTObjectInput in) throws Exception {
				...
				DEBUG.OUT("deserialise biary SerialisedAgent ");
				int one = in.readInt();
				DEBUG.OUT("one " + one);
				...
				String uuid = in.readStringUTF();
				DEBUG.OUT("fivefivefivefivefive1 " + uuid);
				UUID five = UUID.fromString(uuid);;
				DEBUG.OUT("fivefivefivefivefive2 " + five);
				
				return new SerialisedAgent(one, two, three, four, five);
			}
		});
		
		register(conf, UUID.class, new FSTIndividualSerialiser<UUID>() {

			@Override
			public void serialise(final FSTObjectOutput out, final UUID o) throws Exception {
				DEBUG.OUT("serialise UUID " + o.toString());
				out.writeStringUTF(o.toString());
			}

			@SuppressWarnings ("unchecked")
			@Override
			public UUID deserialise(final IScope scope, final FSTObjectInput in) throws Exception {
				DEBUG.OUT("deserialise UUID ");
				String uuid = in.readStringUTF();
				DEBUG.OUT("uuiduuiduuid uuid " + uuid);
