...
public interface IPopulation<T extends IAgent>
	 ...
	 * @date 6 août 2023
	 */
	T getOrCreateAgent(final IScope scope, final Integer index, Map<String, Object> initValues);

	/**
	 * Same as the getOrCreateAgent, just checking if the existing agent match the uuid in parameter, if not we create a "blank" agent
	 *
	 ...
	 */
	T getOrCreateAgent(final IScope scope, final int index, Map<String, Object> initValues, UUID uuid);
